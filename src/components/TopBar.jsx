...
// ============================================
// FUNDS MODAL COMPONENTS - Transparent/Glass, theme‑aware
// ============================================
...
const ModalCard = styled.div`
  ...
  border-radius: 24px;
  box-shadow: ${p => p.theme.colors?.shadow || '0 20px 60px rgba(0,0,0,0.5)'};
...
const TopBar = styled.header`
  ...
  @media (max-width: 1024px) { padding: 12px 20px; flex-wrap: wrap; gap: 12px; }
  @media (max-width: 768px) { flex-direction: column; align-items: stretch; padding: 12px 16px; }
`;

const LeftSection = styled.div`
  display: flex; align-items: center; gap: 16px; flex-shrink: 0;
  @media (max-width: 768px) { width: 100%; justify-content: flex-start; }
`;
...
const SidebarToggle = styled.button`
  ...
  transition: all 0.25s cubic-bezier(0.16, 1, 0.3, 1); padding: 0; flex-shrink: 0;
  ...
  .line {
    display: block; height: 2px; background: ${props => props.theme?.colors?.text || '#ffffff'};
    border-radius: 4px; transition: all 0.3s cubic-bezier(0.34, 1.56, 0.64, 1);
    &:nth-child(1) {
      width: 18px;
...
const Brand = styled.div`
  ...
  .brand-text { display: flex; align-items: center; gap: 4px; letter-spacing: -0.4px; }
  .voltix { color: ${props => props.theme?.colors?.text || '#ffffff'}; }
  .deriv { color: #ff444f !important; font-style: italic; font-weight: 900; letter-spacing: -0.2px; }
  .live-dot {
...
const DropdownContainer = styled.div` position: relative; display: inline-block; `;
...
const GlassDropdownMenu = styled.div`
  ...
  min-width: 300px; max-width: 90vw; max-height: 450px; overflow-y: auto;
  background: ${props => props.theme?.colors?.dropdownBg || 'rgba(15, 23, 42, 0.96)'};
  backdrop-filter: blur(24px) saturate(190%);
  -webkit-backdrop-filter: blur(24px) saturate(190%);
  border: 1px solid ${props => props.theme?.colors?.dropdownBorder || 'rgba(255,255,255,0.08)'};
  border-radius: 18px; padding: 8px;
  box-shadow: ${props => props.theme?.colors?.dropdownShadow || '0 25px 50px -12px rgba(0,0,0,0.5)'};
  opacity: ${props => props.isOpen ? 1 : 0};
  visibility: ${props => props.isOpen ? 'visible' : 'hidden'};
  transform: ${props => props.isOpen ? 'translateY(0) scale(1)' : 'translateY(-8px) scale(0.97)'};
  transition: all 0.25s cubic-bezier(0.16, 1, 0.3, 1); z-index: 300; overflow: hidden;

  &::before {
    content: ''; position: absolute; top: 0; left: 0; right: 0; height: 2px;
    background: linear-gradient(90deg, transparent, ${p => p.theme.colors?.accent || '#3B82F6'}, transparent);
    opacity: 0.6; border-radius: 18px 18px 0 0;
  }
  &::-webkit-scrollbar { width: 3px; }
  &::-webkit-scrollbar-thumb { background: ${props => props.theme?.colors?.scrollbar || 'rgba(255,255,255,0.15)'}; border-radius: 10px; }
`;

const MenuHeader = styled.div`
  padding: 8px 12px 10px; font-size: 10px; font-weight: 700; text-transform: uppercase;
  letter-spacing: 0.8px; color: ${props => props.theme?.colors?.textMuted || '#94a3b8'};
...
const ThemeButton = styled.button`
  ...
  @media (max-width: 480px) { padding: 5px 10px; .theme-name { display: none; } }
`;
...
// ============================================
// 2. FUNDS BUTTON & OPTIONS (no arrow, theme-aware)
// ============================================
const FundsButton = styled.button`
  ...
  .funds-sub { font-size: 9px; color: ${props => props.theme?.colors?.textMuted || '#94a3b8'}; font-weight: 500; letter-spacing: 0.2px; }
  .arrow { display: flex; align-items: center; transition: transform 0.3s ease, color 0.3s ease; opacity: 0.6; margin-left: 2px; }
  @media (max-width: 480px) { padding: 5px 10px; .funds-sub { display: none; } .funds-title { font-size: 11px; } }
`;
...
const FundsOption = styled.div`
  ...
  .fund-name { font-weight: 700; margin-bottom: 1px; }
...
// ============================================
// ... (COUNTRY_CURRENCIES array remains identical)
const COUNTRY_CURRENCIES = [
  { code: 'USD', flag: '🇺🇸', name: 'US Dollar', symbol: '$' },
  // ... (all currencies unchanged)
...
const AccountBadge = styled.div`
  ...
  user-select: none; font-size: 12.5px; font-weight: 700;
  color: ${props => props.theme?.colors?.text || '#ffffff'};
...
const SearchInput = styled.input`
  ...
  color: ${props => props.theme?.colors?.text || '#ffffff'};
  font-size: 12px; font-weight: 500; outline: none; transition: all 0.2s ease; margin-bottom: 4px;
  &:focus {
    border-color: ${props => props.theme?.colors?.accent || '#3b82f6'};
    box-shadow: 0 0 0 3px ${props => (props.theme?.colors?.accent || '#3b82f6') + '15'};
  }
  &::placeholder { color: ${props => props.theme?.colors?.textMuted || '#64748b'}; font-weight: 400; }
`;
...
const CurrencyList = styled.div`
  ...
  &::-webkit-scrollbar { width: 3px; }
  &::-webkit-scrollbar-thumb { background: ${props => props.theme?.colors?.scrollbar || 'rgba(255,255,255,0.1)'}; border-radius: 10px; }
`;
...
const ExitButton = styled.button`
  ...
  @media (max-width: 480px) { padding: 5px 10px; font-size: 11px; .exit-icon { width: 14px; height: 14px; } }
`;
...
// ============================================
// MAIN COMPONENT (Transfer removed, colours themed)
// ============================================
const TopPanel = ({ 
  isSidebarOpen, 
  onSidebarToggle, 
  ...
  const toggleDropdown = () => { setIsDropdownOpen(!isDropdownOpen); setIsThemeOpen(false); setIsFundsOpen(false); };
  const toggleThemeDropdown = () => { setIsThemeOpen(!isThemeOpen); setIsDropdownOpen(false); setIsFundsOpen(false); };
  const toggleFundsDropdown = () => { setIsFundsOpen(!isFundsOpen); setIsDropdownOpen(false); setIsThemeOpen(false); };
  ...
  // Render modal content – 'transfer' case removed
